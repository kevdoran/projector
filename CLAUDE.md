...
| `internal/git` | Thin wrappers around the `git` executable: `RunGit`, `WorktreeAdd`, `WorktreeAddDetached`, `WorktreeRemove`, `WorktreeList`, `StatusPorcelain`, `RefExists`, `BranchExists`, `CurrentBranch`, `AvailableBranchName`, `BranchNameFromRef`, `Remotes`, `RemoteForRef`, `Fetch`, `HasUnpushedCommits`, `HeadSHA`, `MinVersionCheck`. |
